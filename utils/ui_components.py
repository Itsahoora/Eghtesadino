    ...
    def __init__(self, radius=14, bg_color='card_bg', hover=False, **kwargs):
        ...
        try:
            self.configure(fg_color=color, corner_radius=sv(radius),
                           border_width=1, border_color=self._rest_border)
        except Exception:
...
class ProgressBar(ctk.CTkFrame):
    ...
    def __init__(self, master, height=8, fg_color='divider',
                 progress_color='accent', duration_ms=260, **kwargs):
        super().__init__(master, fg_color='transparent', **kwargs)
        self._height = height
        ...
        self._bg = ctk.CTkFrame(self, fg_color=self._fg_color,
                                corner_radius=radius)
        self._bg.pack(fill='both', expand=True)

        self._fill = ctk.CTkFrame(self._bg, fg_color=self._progress_color,
                                  corner_radius=radius, width=0)
    ...
    def _animate_to(self, start, target, steps, step):
        ...
        if step < steps - 1:
            try:
                self._after_id = self.after(16, lambda: self._animate_to(
                    start, target, steps, step + 1))
            except Exception:
                self._progress = target
                self._fill.place_configure(relwidth=target)
...
class Donut(ctk.CTkFrame):
    def __init__(self, master, size=72, progress_color='accent',
                 track_color='divider', thickness=7, **kwargs):
        ...
        self._canvas = tk.Canvas(
            ...
            bg=COLORS.get('card_bg'), highlightthickness=0, bd=0,
        )
        self._canvas.pack()
    ...
    def _redraw(self):
        ...
        self._canvas.create_arc(
            *bbox, start=90, extent=359.999, style='arc',
            outline=self._track, width=self._thickness)
        extent = -360.0 * self._frac
        if extent <= -0.01:
            self._canvas.create_arc(
                *bbox, start=90, extent=extent, style='arc',
                outline=self._color, width=self._thickness)
    ...
    def _animate_to(self, start, target, steps, step):
        ...
        if step < steps - 1:
            try:
                self.after(16, lambda: self._animate_to(
                    start, target, steps, step + 1))
            except Exception:
                self._frac = target
                self._redraw()
...
class ScreenHeader(ctk.CTkFrame):
    ...
    def __init__(self, master, icon, title, on_back=None, **kwargs):
        ...
        if on_back:
            ctk.CTkButton(
                left, text='←  Back', command=on_back,
        ...
        ctk.CTkLabel(
            right, text=icon, font=font(18), text_color=COLORS.get('primary'),
        ).pack(side='left', padx=(0, sv(6)))

        ctk.CTkLabel(
            right, text=title, font=font(20, 'bold'), text_color=COLORS.get('text'),
        ).pack(side='left')
