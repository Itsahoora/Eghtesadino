...
class FinancialFeatureTests(unittest.TestCase):
    ...
    def test_allocate_to_goal_invalidates_cache(self):
        ...
        self.assertEqual(
            self.advisor.get_goal_progress(user['id'])[0]['current'], 200)
    ...
    def test_encrypted_fields_round_trip_and_salt_stable(self):
        ...
        stored = self.advisor._connect().execute(
        ...
        self.assertEqual(self.advisor._get_user_salt(user['id']),
    ...
    def test_cache_isolated_per_user(self):
        ...
        self.assertEqual(
            len(self.advisor.get_transactions(bob['id'], days=30)), 1)
