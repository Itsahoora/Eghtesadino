...
class DashboardScreen(BaseScreen):
    ...
    def _build_balance_card(self, med):
        ...
        self.balance_card = Card(master=self.content, radius=sv(16), bg_color="hero_bg")
        self.balance_card.pack(fill="x", pady=(0, med))

        card_fill = COLORS.get("hero_bg")
        try:
            self.balance_card.configure(
                fg_color=card_fill, border_color=COLORS.get("divider")
            )
        except Exception:
            pass
    ...
    def _build_stats_row(self, med):
        ...
        self.stat_daily = self._make_stat_card(
            self.stats_frame, ICONS["expense"], "Daily avg", "--"
        )
        self.stat_daily.pack(side="left", fill="x", expand=True, padx=(0, sv(6)))

        self.stat_savings = self._make_stat_card(
            self.stats_frame, ICONS["tips"], "Savings rate", "--"
        )
        self.stat_savings.pack(side="left", fill="x", expand=True, padx=sv(3))

        self.stat_velocity = self._make_stat_card(
            self.stats_frame, ICONS["forward"], "Spending trend", "--"
        )
    ...
    def _build_goals_card(self, med):
        ...
        self.goals_list_frame = ctk.CTkFrame(self.goals_card, fg_color="transparent")
    ...
    def _build_transactions_card(self, med):
        ...
        self.trx_list = ctk.CTkTextbox(
            ...
            fg_color=COLORS.get("input_bg"),
            text_color=COLORS.get("text"),
            font=(FONT_FAMILY, sv(12)),
            border_width=0,
        )
        self.trx_list.pack(fill="both", expand=True, padx=sv(20), pady=(0, sv(16)))
    ...
    def _add_transaction(self):
        ...
        self.advisor.add_transaction(self.user_id, ttype, cat, amount, description=desc)
    ...
    def _do_allocate(self):
        ...
        success, msg = self.advisor.allocate_to_goal(self.user_id, goal_id, amount)
        if success:
    ...
    def _render_goals(self):
        ...
        if goal_names:
            ...
            if (
                not self.goal_var.get()
                or self.goal_var.get() == "No goals yet. Add one to start!"
            ):
        ...
        for g in goal_progress:
            ...
            status_icon = (
                ...
                else (ICONS["warning"] if g["status"] == "almost" else ICONS["goal"])
            )

            ctk.CTkLabel(
                name_frame,
                text=f"{status_icon}  {g['name']}",
                font=font(12, "bold"),
                text_color=text_color,
            ).pack(anchor="w")

            detail = (
                f'{g["current"]:,.0f} / {g["target"]:,.0f}  ({g["percentage"]:.1f}%)'
            )
    ...
    def refresh_ui(self, user_id=None):
        ...
        try:
            ...
            if bal < 0:
                self._set_stat(self.stat_savings, "Deficit", COLORS.get("warning"))
        ...
        try:
            ...
            if advice is None:
                MutedLabel(
                    self.coach_body,
                    text=(
                        f"{ICONS['info']}  Start by adding an income and an "
