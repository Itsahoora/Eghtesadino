...
class ReportsScreen(BaseScreen):
    ...
    def _kpi_row(
        ...
        ctk.CTkLabel(row, text=left_label, font=font(11), text_color=muted).grid(
            row=0, column=0, sticky="w"
        )
        ctk.CTkLabel(
            row, text=left_val, font=font(13, "bold"), text_color=left_color or default
        ).grid(row=1, column=0, sticky="w", pady=(sv(2), 0))

        ctk.CTkLabel(row, text=right_label, font=font(11), text_color=muted).grid(
        ...
        ctk.CTkLabel(
            ...
            text=right_val,
            font=font(13, "bold"),
            text_color=right_color or default,
        ).grid(row=1, column=1, sticky="w", padx=(sv(16), 0), pady=(sv(2), 0))
    ...
    def _change_chip(self, parent, label, value, positive_good=True):
        ...
        color = (
            COLORS.get("success")
            if (value >= 0) == positive_good
            else COLORS.get("danger")
        )
        prefix = "+" if value > 0 else ""
        return ctk.CTkLabel(
            parent,
            text=f"{prefix}{value:.1f}% {label}",
            font=font(10, "bold"),
            text_color=color,
            fg_color=COLORS.get("input_bg"),
            corner_radius=sv(6),
    ...
    def _trend_chart(self, parent, trend):
        ...
        for i, m in enumerate(trend):
            ...
            for j, (val, color) in enumerate(
                ...
                canvas.create_rectangle(
                    x0, y0, x0 + bar_w, pad_t + plot_h, fill=color, outline="", width=0
                )
        ...
        legend_y = sv(6)
        canvas.create_rectangle(
            sv(110), legend_y, sv(122), legend_y + sv(8), fill=income_col, outline=""
        )
        canvas.create_text(
            sv(126),
            legend_y + sv(4),
        ...
        canvas.create_rectangle(
            sv(180), legend_y, sv(192), legend_y + sv(8), fill=expense_col, outline=""
        )
        canvas.create_text(
            sv(196),
            legend_y + sv(4),
    ...
    def _category_bars(self, parent, cats):
        ...
        for cat in cats[:8]:
            ...
            ctk.CTkLabel(
                row,
                text=f'{ICONS["expense"]} {cat["category"]}',
                font=font(11),
                text_color=COLORS.get("text"),
                width=sv(140),
                anchor="w",
            ).pack(side="left")
            bar = ProgressBar(row, height=sv(8), width=sv(120), progress_color="info")
    ...
    def refresh_ui(self, user_id=None):
        ...
        try:
            ...
            self._kpi_row(
                ...
                f'{prev["income"]:,.2f}',
                left_color=success_col,
                right_color=success_col,
            )
            self._kpi_row(
                body,
                ...
                f'{prev["expense"]:,.2f}',
                left_color=danger_col,
                right_color=danger_col,
            )
            self._kpi_row(
                body,
            ...
            ctk.CTkLabel(
                chg_row, text="Change:", font=font(11, "bold"), text_color=muted_col
        ...
        try:
            ...
            if not recurring:
                ...
                for r in recurring:
                    row = ctk.CTkFrame(body, fg_color=input_bg, corner_radius=sv(8))
                    ...
                    ctk.CTkLabel(
                        inner,
                        text=f'{ICONS["repeat"]}  {r["category"]}',
                        font=font(12, "bold"),
                        text_color=text_col,
                    ).pack(side="left")
                    ...
                    ctk.CTkLabel(
                        inner, text=detail, font=font(11), text_color=muted_col
                    ).pack(side="left")
        except Exception:
            self._empty_state(body, "No data")
        ...
        try:
            ...
            if not si or si["total_expense"] == 0:
                ...
                self._kpi_row(
                    body,
                    "Daily Average",
                    f'{si["daily_average"]:,.0f}',
                    "Projected Month End",
                    f'{si["projected_monthly"]:,.0f}',
                ...
                if bal < 0:
                    ...
                    self._kpi_row(
                        body,
                        "Savings Rate",
                        f'{si["savings_rate"]:.1f}%',
                        "Transactions",
                        str(si["transaction_count"]),
                        left_color=(
                            success_col if si["savings_rate"] >= 20 else danger_col
                        ),
                    )
                ...
                vel_frame = ctk.CTkFrame(body, fg_color=input_bg, corner_radius=sv(8))
                ...
                ctk.CTkLabel(
                    vel_inner,
                    text=f"{vel_icon}  Spending Velocity: ",
                    font=font(12, "bold"),
                ...
                if si["top_categories"]:
                    ...
                    for cat_name, cat_amt in si["top_categories"][:3]:
                        cat_pct = (
                            (cat_amt / si["total_expense"] * 100)
                            if si["total_expense"] > 0
                            else 0
                        )
                        ctk.CTkLabel(
                            body,
                            text=f"    {cat_name:16}  {cat_amt:>10,.0f}  ({cat_pct:.1f}%)",
                            font=(FONT_FAMILY, sv(11)),
                            text_color=text_col,
                        ).pack(anchor="w")
        except Exception:
